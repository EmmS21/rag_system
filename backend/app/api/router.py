...
@router.post("/query/")
async def receive_query(query: ExtendedQuery):
    query_id = str(uuid4())
...
async def stream_response(query_id: str):
    ...
    return StreamingResponse(response_generator.generate(context["combined_texts"], context["original_query"], context.get("context")), media_type="text/event-stream")
